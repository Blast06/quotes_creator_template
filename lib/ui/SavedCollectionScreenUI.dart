...
class SavedCollectionScreenUI extends StatefulWidget {
  @override
  _SavedCollectionScreenUIState createState() =>
      _SavedCollectionScreenUIState();
}
...
class _SavedCollectionScreenUIState extends State<SavedCollectionScreenUI> {
  ...
  final SavedCollectionController savedCollectionController =
  ...
  getDataFromDir() async {
    if (Platform.isAndroid) {
      Directory externalDirectory =
          new Directory("/storage/emulated/0/ThoughtsOfTheDay/");

      if (externalDirectory.existsSync()) {
        _files =
            externalDirectory.listSync(recursive: true, followLinks: false);
        ...
        var myDir = new Directory('/storage/emulated/0/ThoughtsOfTheDay/')
      ...
      if (appDocumentsDirectory.existsSync()) {
        _files =
            appDocumentsDirectory.listSync(recursive: true, followLinks: false);
        for (FileSystemEntity entity in _files) {
          String path = entity.path;
          print("PATH " + path);
        }
          ...
          body: new Column(
            children: [
              Expanded(
                flex: 1,
                child: Container(
                  child: controller.imgFiles != null &&
                          controller.imgFiles.length > 0
                      ...
                      : new Center(
                          child: new Text(
                            "Collection is empty!",
                            style: TextStyle(
                                fontSize: 24.0, color: AppColors.primaryColor),
                          ),
                        ),
                ),
              ),
...
//
  ...
  Widget sliverGridWidget(
      BuildContext context, index, SavedCollectionController controller) {
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          decoration: BoxDecoration(
              ...
              border: Border.all(
                  width: controller.imgFiles[index].isSelect ? 2 : 1.5,
                  style: BorderStyle.solid,
                  color: controller.imgFiles[index].isSelect
                      ? AppColors.redColor
              ...
              child: InkWell(
                  child: Hero(
                    ...
                    child: new FadeInImage(
                      ...
                      image: FileImage(
                          controller.imgFiles[index].fileSystemEntity,
                          scale:
                              1.0), // NetworkImage(staticData[index].images),
                      fit: BoxFit.cover,
                      placeholder: AssetImage(
                          controller.imgFiles[index].fileSystemEntity.path),
                    ),
                  ),
                  onTap: () {
                    //
                    if (controller.selectedList.isEmpty) {
                      ...
                      for (int i = 0; i < controller.imgFiles.length; i++) {
                        tmpFilesArray
                            .add(controller.imgFiles[i].fileSystemEntity);
                      }
